...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($nombre === '') {
    ...
    try {
      $st = $conn->prepare("INSERT INTO afps (Nombre) VALUES (?)");
      $st->execute([$nombre]);
      $nuevoId = $conn->lastInsertId();
      registrarAuditoria($conn, (int)$_SESSION['usuario']['id'], 'afps', $nuevoId, 'INSERT', null, ['Nombre'=>$nombre]);
      $msg = ['type'=>'ok','text'=>'AFP creada.'];
...
$rows = $conn->query("SELECT Id_afp, Nombre, Activo, Creado_en FROM afps ORDER BY Nombre")->fetchAll();
?>
<h2>Registrar AFP</h2>
<?php if ($msg): ?>
