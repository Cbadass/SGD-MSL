...
// 1) Capturar Id_profesional
$id = intval($_GET['Id_profesional'] ?? 0);
if ($id <= 0) {
    die("ID inválido.");
}

// 2) Traer datos del profesional y su usuario
$stmt = $conn->prepare("
...
// — Las mismas funciones y arrays que en create_profesional.php —
function cleanRut($rut){return preg_replace('/[^0-9kK]/','',$rut);}
function dvRut($rut){ /*...*/ }
...
$cargos      = ['Administradora','Directora','Profesor(a) Diferencial','Profesor(a)',
...
<body class="p-4">
  ...
  <form method="POST" action="guardar_modificacion_profesional.php" class="row g-3 needs-validation" novalidate>
    ...
    <!-- Repite todos los campos de create_profesional.php, pero con value="<?= htmlspecialchars($prof['campo']) ?>" -->
    <div class="col-md-6">
    ...
    <div class="col-md-4">
      ...
      <select name="tipo_profesional" class="form-select" required>
        ...
        <?php foreach ($tipos_prof as $t): ?>
          <option <?= $prof['Tipo_profesional']==$t?'selected':'' ?>><?= htmlspecialchars($t) ?></option>
        <?php endforeach ?>
      </select>
    </div>
    <div class="col-md-4">
    ...
    <div class="col-md-4">
      ...
      <select name="estado_civil" class="form-select" required>
        ...
        <?php foreach ($estados as $val => $lbl): ?>
          <option value="<?= $val ?>"
            <?= $prof['Estado_civil_profesional']==$val?'selected':'' ?>>
