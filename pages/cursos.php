...
$idsCursosPermitidos = $alcance['cursos'];
// ====================================
...
// Escuela (solo ADMIN puede filtrar, DIRECTOR ve solo la suya automáticamente)
...
echo " <button type='button' class='btn btn-secondary btn-height' onclick=\"window.location='?seccion=cursos'
    \">Limpiar</button>";
echo "</form>";

// Construir consulta con filtro de alcance
$sql = "
SELECT
c.Id_curso,
c.Tipo_curso,
...
WHERE " . filtrarPorIDs($idsCursosPermitidos, 'c.Id_curso');

$params = [];
agregarParametrosFiltro($params, $idsCursosPermitidos);
...
if ($filtro_grado !== '') {
    $sql .= " AND c.Grado_curso = ?";
    $params[] = $filtro_grado;
}
if ($filtro_seccion !== '') {
    $sql .= " AND c.seccion_curso = ?";
...
if ($cursos) {
    foreach ($cursos as $r) {
        ...
        if (in_array($alcance['rol'], ['ADMIN', 'DIRECTOR'], true)) {
            echo "<a href='index.php?seccion=modificar_curso&Id_curso={$r[' Id_curso']}'
